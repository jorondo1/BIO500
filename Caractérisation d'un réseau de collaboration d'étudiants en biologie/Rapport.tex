...
\title{Caractérisation d'un réseau de collaboration d'étudiants en
biologie.}

\author[a]{Amélie Harbeck-Bastien}
\author[a]{Samuel Fortin}
\author[a]{Jonathan Rondeau-Leclaire}
...
\begin{abstract}
``Le concept de réseaux écologiques a été décrit et étudié en profondeur
dans la littérature (\textbf{delmas\_analysing\_2019?}). Or, il serait
intéressant d'évaluer si les propriétés d'un réseau de collaboration
humain dans un contexte académique peut se comparer à celles des réseaux
écologiques. Pour ce faire, le coefficient de regroupement d'étudiants
au baccalauréat (BAC) en biologie a servi à déterminer si, dans un
contexte de collaborations lors de travaux d'équipes, les étudiants
avaient tendance à former des regroupements spécifiques de manière
fréquente au cours de leur parcours universitaire. Il en résulte un
phénomène de regroupement plus prononcé selon l'année de début du BAC,
démontrant ainsi qu'il existe une facteur lié à l'année d'entrée à
l'université qui semble augmenter la valeur adaptative des étudiants
dans cet environnement relativement stressant et compétitif.''
\end{abstract}
