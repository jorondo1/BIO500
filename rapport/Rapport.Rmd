...
author:
  ...
  - name: "Jonathan Rondeau-Leclaire"
    institute: udes
institute:
  - udes: "Université de Sherbrooke, Département de biologie, 2500, Boulevard de l'Université, Sherbrooke, Québec, J1K 2R1"
date: "22 avril 2023"
abstract: "Le concept de réseaux écologiques a été décrit et étudié en profondeur dans la littérature [@delmas_analysing_2019]. Or, il serait intéressant d'évaluer si les propriétés d'un réseau de collaboration humain dans un contexte académique peut se comparer à celles des réseaux écologiques. Pour ce faire, le coefficient de regroupement d'étudiants au baccalauréat (BAC) en biologie a servi à déterminer si, dans un contexte de collaborations lors de travaux d'équipes, les étudiants avaient tendance à former des regroupements spécifiques de manière fréquente au cours de leur parcours universitaire. Il en résulte un phénomène de regroupement plus prononcé selon l'année de début du BAC, démontrant ainsi qu'il existe une facteur lié à l'année d'entrée à l'université qui semble augmenter la valeur adaptative des étudiants dans cet environnement relativement stressant et compétitif."
...
```{r fig1, Reseau interactif, fig.cap="\\label{fig:fig1} Réseau de collaboration entre les étudiants de la classe du cours BIO500 à l'Université de Sherbrooke à l'hiver 2023.", fig.width=9.5, fig.height=6, echo=FALSE}
...
```{R tab1, tab.cap="\\label{tab:tab1} Tableau 1. Caractéristiques générales du réseau global et du sous-réseau du noyau des étudiants de la classe.", echo=FALSE}
...
```{R fig2, Boxplots, fig.cap="\\label{fig:fig2}Comparaison de la distribution des coefficients de regroupements individuels par caractéristique du parcours des personnes étudiantes.", fig.width=9, fig.height=9, message=FALSE, warning=FALSE, echo=FALSE}
...
```{R "Visualisation du réseau en fonction de l'année de début", message=FALSE, warning=FALSE, echo=FALSE}
